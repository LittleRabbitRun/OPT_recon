                                    ...
                                    long psfn3, long psfn2, long psfn1)
{
  ...
  // reduce to 2D problem (d3 = [ c00  c01
  //                              c10  c11]
  c00 = c100 - c000;
  c01 = c101 - c001;
  c10 = c110 - c010;
                      ...
                      double psfstart3, double psfstart2, double psfstart1) 
{
  ...
  start = -FOV/2 */
  printf("Attempting to build psf in fdr space filter\n");
...
int deconvolution_in_fdr_space_filter(mihandle_t fdrfile, mihandle_t deconvfile, long n3, long n2, long n1, double noise_reduction) 
{
  long k,i;
  unsigned long start[3], count[3];
  int result;
  ...
  printf("Attempting to build deconvolution in fdr space filter\n");
  ...
  for(k=0;k<n3;k++)
  {
   ...
   {
    tmpv = fdrdata[i];
    // attempting to calculate the following in one shot
    // h*/(abs(h)+n**2)
        tmp  = /*(fabs)^2(*/creal(tmpv)*creal(tmpv)+cimag(tmpv)*cimag(tmpv); // sqrt*sqrt-->tmp
    // oct 2/2012 removed n**n as discussed with JGSLED
    deconvdata[i] = (creal(tmpv)+(-1*cimag(tmpv)*I))/(tmp+n*n);
    //deconvdata[i] = (creal(tmpv)+(-1*cimag(tmpv)*I))/tmp;

     }
   result = miset_voxel_value_hyperslab(deconvfile,MI_TYPE_FCOMPLEX,start,count,(mifcomplex_t *)deconvdata);
   if(result == MI_ERROR)
   {
    fprintf(stdout, "Could not load the entire FDR file\n");
    fftwf_free(fdrdata); 
                   ...
                   long n3, long n2, long n1, double step2, double step1) 
{
  ...
  rolloff_filter = (float *)fftwf_malloc(n2*n1*sizeof(float)); 

  printf("Attempting to build rolloff filter\n"); 
  for(k=0;k<n3;k++) 
  {
   //for(i=0;i<n1;i++) 
   for (j=0; j < n2; j++)
    {
     ...
     //rx  = starti + i*stepi;
     rx=startj+j*stepj;
     if(fabs(rx)> 1e-9)  // avoid division by zero
      {
      l = phi/rx;
      if (l <= 0)
       {
      ...
      else if ( l < weight && l > 0)
       {
        tmp = pow(cos(M_PI/2.0*fabs(l)/weight),2);
       }  
      else if ( l > weight)
       { 
        tmp = 0.0;
     ...
     //writing data in more elegant way to avoid cache line slow downs
     for(i=0;i<n1;i++)
        rolloff_filter[j*n1+i]=tmp;
...
int bandlimit_filter(mihandle_t inputfile, double bandlimit, long n3, long n2, long n1,double step2, double step1)
{
 ...
 printf("Attempting to build bandlimit filter\n"); 
 for(k=0;k<n3;k++) 
 {
  for(j=0;j<n2;j++) 
  {
...
int limit_recovery_filter(mihandle_t inputfile, mihandle_t outputfile, double fdrlimit, long n3, long n2, long n1) 
{
 ...
 printf("Attempting to build limit recovery filter\n");
