...
: proj(proj), im(im), interpType(interpType), m_interpFactor(interpFactor), m_bPostProcessingDone(false)
{
  detInc = proj.detInc();
  nDet = proj.nDet();
  iDetCenter = (nDet - 1) / 2.0 - proj.rotOffset();	// index refering to L=0 projection 
  std::cout << "iDetCenter is" << iDetCenter << std::endl;
  std::cout << "rotoffset is" << proj.rotOffset() << std::endl;
...
BackprojectDiff::BackprojectView (const double* const filteredProj, const double view_angle)
{
  ...
  //cout << "DPCS is " << detPosColStart << endl;
  //cout << "iDC  is " << iDetCenter << endl;
